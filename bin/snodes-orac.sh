...
scontrol show nodes | grep -A8 -e "NodeName" | grep -v "BootTime=" | grep -v "Partitions=" | grep -v "Gres" | grep -v "NodeAddr" | grep -v "Watts" | grep -v "^--" | sed "s/OS=Linux //g" | grep -v "ActiveFeatures" | grep -v "AvailableFeatures" | awk 'BEGIN{FS=" |="; OFS="\t";
